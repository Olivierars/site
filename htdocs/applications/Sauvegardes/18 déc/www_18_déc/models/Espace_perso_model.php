...
// TRAJET ----------------------------------------------------
	...
	{
		$table = 'trajet';
		return $this->db->select('`id`,`ville_depart`, `ville_arrivee`,`date_trajet`' ,false)  // DATE_FORMAT(`date_trajet`,\'%d/%m/%Y \') AS \'date_trajet\'
				->from($table)
	...
	{
		$table = 'candidat_vs_trajet';
		return $this->db->select('`id`,`id_trajet`,`statut`',false)
			-> from($table)
