...
class Messagerie extends CI_Controller {
  ...
  public function messages_en_cours($page = 'messages_en_cours')
  {
    ...
    if($this->session->userdata('logged_in'))
    {
      ...
      // Boucle sur tous les interlocuteurs avec qui j'echange pour récupérer les trajets
  ...
  public function messages_archives($page = 'messages_archives')
  {
   ...
   public function conversation($id_message)
  {
    ...
    if($this->session->userdata('logged_in'))
    {
      ...
      // Obtention des datas du message
      $data_message   = $this->Espace_persoManager->get_messages($id_message);

      // Obtention des datas du trajet get_trajet
      $id_trajet      = $data_message[0]->id_trajet ; 
      $data['data_trajet']  = $this->Espace_persoManager->get_trajet($id_trajet);

      // Obtention des datas de l'interlocuteur 
      if ($data_message[0]->id_dest == $data['id']) 
      {
        $id_interloc  = $data_message[0]->id_exped;
      } 
      elseif ($data_message[0]->id_exped == $data['id']) 
      {
        $id_interloc  = $data_message[0]->id_dest;
      }
      $data['interloc']  = $this->Espace_persoManager->get_profil($id_interloc);

      // Obtention des messages liés à cet interloc et ce trajet 
      $liste_id_messages_brute = $this->Espace_persoManager->get_id_messages($data['id'],$id_interloc,$id_trajet);
      ...
      for($i = 0 ; $i <= $Nof_message_brut-1 ; $i++)
      {
        if(!in_array($liste_id_messages_brute[$i]->id, $liste_id_messages))
        {
          $liste_id_messages[] = $liste_id_messages_brute[$i]->id ;
          $data_messages[]      = $this->Espace_persoManager->get_messages( $liste_id_messages_brute[$i]->id );
        }
      }  

      // Chargement de le page html
      $data['liste_id_messages'] = $liste_id_messages ;
      $data['data_message'] = $data_messages ;
      $page = 'conversation';
      $data['title'] = ucfirst($page); // Capitalize the first letter
      $data['description'] = 'Messages en cours';
      $data['menu'] = 'messagerie' ;
  ...
  public function repondre()
  {
    ...
    if($this->session->userdata('logged_in'))
    {
      ...
      if($this->form_validation->run())
      {
        ...
        // print_r($data_message);
        $id_insert_message = $this->Espace_persoManager->ajouter_message($data_message);
      }
     
      // redirect('Espace_perso/Messagerie/messages_en_cours', 'refresh');
      redirect('Espace_perso/Messagerie/conversation/'.$id_insert_message, 'refresh');
