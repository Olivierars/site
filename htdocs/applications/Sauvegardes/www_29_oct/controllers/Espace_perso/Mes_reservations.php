...
class Mes_reservations extends CI_Controller {
  ...
  public function attente_confirmation($page = 'attente_confirmation')
  { 
    if($this->session->userdata('logged_in'))
    {
      ...
      if (!empty($ids_trajet))
      {
        ...
        {
          $trajet_en_attente[]    = $this->Espace_persoManager->get_trajet($ids_trajet[$i]->id_trajet);
  ...
  public function confirme($page = 'confirme')
  { 
    if($this->session->userdata('logged_in'))
    {
      // Récupération des donnée du compte
      ...
      // Récupération des trajets confirmés
